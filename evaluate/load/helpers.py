...
def n_grams(tokens, n=1):
    ...
    """
    def shiftToken(i): return (el for j, el in enumerate(tokens) if j >= i)
...
def hashing_trick(txt,
    ...
    if hash_function is None:
        ...
        def hash_function(w): return int(hashlib.md5(w.encode('utf-8')).hexdigest(), 16)
    elif hash_function == 'sha1':
        def hash_function(w): return int(hashlib.sha1(w.encode('utf-8')).hexdigest(), 16)
